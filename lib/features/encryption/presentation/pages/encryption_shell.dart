...
class _EncryptionShellState extends State<EncryptionShell> {
  ...
  Future<void> _checkForSecurityAlert() async {
    ...
    try {
      ...
      final latestIncident = Map<String, dynamic>.from(incidents.first as Map);

      final latestIncidentId = int.tryParse(latestIncident['id'].toString());

      if (latestIncidentId == null) {
        return;
      }

      final rawCreatedAt = latestIncident['created_at']?.toString();

      final securityWindowStartedAt =
          SessionService.getSecurityWindowStartedAt();

      if (rawCreatedAt != null && securityWindowStartedAt != null) {
        ...
        if (incidentTime == null ||
          ...
          await SessionService.saveLastSeenSecurityIncidentId(latestIncidentId);
          return;
        }
      }
                            ...
                            children: [
                              ...
                              ProfilePage(
                                ...
                                onNotificationsPressed: () =>
                                    _openUtilityPage(4),
                                onSettingsPressed: () => _openUtilityPage(5),
                              ),
                              NotificationsPage(
                                onBackPressed: _closeUtilityPage,
                              ),
...
class _Backdrop extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final colors = context.encryptoColors;
    return DecoratedBox(
